...
describe("Pull controller Tests", function ()
{
   ...
   it("Complite pull", async function()
   {
    let userReg = await userController.Register(acc1.address, 1);
    await userReg.wait();
    let tx = await pullController.BuyTicket(2, 500);
    await tx.wait();

    let userReg1 = await userController.Register(acc2.address, 1);
    await userReg1.wait();
    let tx1 = await pullController.BuyTicket(3, 600);
    await tx1.wait();
    ...
    expect(pull[0]).to.eq(1000);
    expect(pull[1]).to.eq(100);
    expect(pull[2]).to.eq(1);
   ...
   expect(ticketeinfo1[0]).to.eq(2);
   expect(ticketeinfo1[1]).to.eq(500);

   expect(ticketeinfo2[0]).to.eq(3);
   expect(ticketeinfo2[1]).to.eq(500);

   expect(ticketeinfo3[0]).to.eq(3);
   expect(ticketeinfo3[1]).to.eq(100);
    ...
    let tx3 = await pullController.BuyTicket(4, 400);
    await tx3.wait();
    let tx4 = await pullController.BuyTicket(5, 500);
    await tx4.wait();
    ...
    expect(pull3[0]).to.eq(1000);
    expect(pull3[1]).to.eq(0);
